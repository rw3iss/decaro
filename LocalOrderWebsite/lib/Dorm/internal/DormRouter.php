<?php
/* DormRouter - basic routing mechanism supporting regex */
...
require_once(DORM_PATH . '/internal/dorm_routes.php');
...
class DormRouter {
	...
	private function _matchRoute($request) {
		...
		if(!$route) {
			//try to show the 404 page
			$route = $this->_match_page_not_found_route($request);
		}
	...
	private function _matchStaticRoute($request) {
		...
		foreach ($this->routes as $key => $val)
		{
			...
			if (preg_match('#^'.$key.'$#', $request->uri) || preg_match('#^'.$key.'$#', '/' . $request->uri) )
			{
				...
				{
					//$val = preg_replace('#^'.$key.'$#', $val, $request->uri);

					//store any parameters
					preg_match_all('#^'.$key.'$#', $request->uri, $matchedParams);

					if(sizeof($matchedParams) > 0) {
						$params = array();
						for($i = 1; $i < sizeof($matchedParams); $i++) {
							array_push($params, $matchedParams[$i][0]);
						}
	...
	private function _match_page_not_found_route($request) {
		$route = null;
	...
	function _validateRoute($route)
	{
		...
		//trim out wildcards:
		//TODO:
		$route->routePath = preg_replace('/\/\$[0-9]/', '', $route->routePath);
	...
	private function _executeRoute($route) {
		...
		if(!method_exists($routeClass, $route->method)) {
			throw new \Dorm\DormException("The route's method does not exist in the file. File: " . 
				$file . ". Method: " . $route->method);
		}

		//Call the route method
		call_user_func_array(array(&$routeClass, $route->method), $route->params);
	}
